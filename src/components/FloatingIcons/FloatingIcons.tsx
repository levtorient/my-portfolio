    ...
    const animate = () => {
      setIcons(prevIcons =>
        prevIcons.map(icon => {
          ...
          const targetOffsetX = distance > 0 ? (dx / distance) * pullStrength : 0
          const targetOffsetY = distance > 0 ? (dy / distance) * pullStrength : 0

          const newOffsetX = icon.offsetX + (targetOffsetX - icon.offsetX) * 0.08
          const newOffsetY = icon.offsetY + (targetOffsetY - icon.offsetY) * 0.08
